...
! function() {
    function n(n) { var t = .035,
            i = n.width(),
            o = i * t;
        $(".content.auto-font-resize", n).css({ "font-size": o + "px" }) } atlasboard.on("widgetInit", function(t, i) { n(i.$widgetContainer) }) }();
! function() { var i = $("#main-container"),
        n = i.outerWidth(),
        t = i.outerHeight();
    $(window).resize(function() { var e = $(window).width() / n,
            o = $(window).height() / t;
        i.css("transform", "scale(" + Math.min(e, o) + ")") }).resize() }();
...
! function() { atlasboard.on("widgetError", function(r, a) { console.error(a.error || a); var s = a.$errorContainer;
        a.data && "disabled" === a.data.error ? s.html('<span class="disabled">DISABLED</span>') : s.html('<span class="error">&#9888;</span>'), s.is(":visible") || (s.siblings().hide(), s.fadeIn()) }) }();
! function() { var i = $("#main-container"),
        n = $(".gridster ul"),
        s = 2 * parseInt(i.css("paddingTop"), 10),
        t = s / 2,
        e = 1080 - i.offset().top - t,
        d = i.width(),
        r = GRID_ROWS,
        a = GRID_COLUMNS,
        g = { w: (d - a * s) / a, h: (e - r * s) / r };
    n.gridster({ widget_margins: [t, t], widget_base_dimensions: [g.w, g.h] }) }();
! function() { atlasboard.on("widgetInit", function(n, e) { var a = new Spinner({ className: "spinner", color: "#f5f5f5", width: 5, length: 15, radius: 25, lines: 12, speed: .7, shadow: !0 }).spin();
        $("<div>").addClass("spinner").append(a.el).appendTo(e.$widgetContainer.parent()) }), atlasboard.on("widgetData", function(n, e) { $(".spinner", e.$widgetContainer.parent()).hide() }), atlasboard.on("widgetError", function(n, e) { $(".spinner", e.$errorContainer.parent()).hide() }) }();