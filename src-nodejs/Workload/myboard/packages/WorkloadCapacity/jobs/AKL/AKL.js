				...
				try {
					...
					for (var i = 0; i < json.length; i++) {
						...
						if (((json[i].schedHR + json[i].cmplHR) - json[i].mouHR) > 0) {
							...
							exvalues.push((json[i].schedHR + json[i].cmplHR) - json[i].mouHR);
							remvalues.push(0);
						}
						else {
							schedvalues.push(json[i].schedHR);
							remvalues.push((json[i].mouHR - (json[i].schedHR + json[i].cmplHR)));
							exvalues.push(0);
						}
					}
					if (json.length == 1) {
					...
					jobCallback(null, { graphData: graphData, title: 'AKL Scheduled Hours', timezone: config.TZ, err: 0 });
				}
				catch (e) {
					/*uncomment this and comment out the second one when no planned outage*/
					jobCallback(null, { err: 1, msg: errVal });
