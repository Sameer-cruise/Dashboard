...
var tzone = data.timezone;
	$('#date_cd_akl').text(" Time - " + moment.tz(dt,tzone).format('HH:mm:ss')+"L");
	$('#date_cd_utc_akl').text(" UTC - " + moment(dt).utc().format('HH:mm:ss')+"Z");
}, 1000);
		var chartUpdateDiv = $("#chartAKL");
		chartUpdateDiv.html("");
		
		var canvasElement = $('<canvas id = "canvasAKL"/>')
...
//------------------------------------------------------------------------------------------------------------------------------  
		...
		var ctx = document.getElementById("canvasAKL").getContext("2d");
