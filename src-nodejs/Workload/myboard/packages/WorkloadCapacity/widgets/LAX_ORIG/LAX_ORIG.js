widget = {
  ...
  onData: function (el, data) {
	...
	//document.getElementById("day1").innerHTML =data.graphData.Dates[0];
	//document.getElementById("day2").innerHTML =data.graphData.Dates[1];
	//document.getElementById("day3").innerHTML =data.graphData.Dates[2];
	//document.getElementById("day4").innerHTML =data.graphData.Dates[3];
   ...
   /* if (data.title) {
	...
	if(data.timezone){
		...
		var tzone = data.timezone;
			...
			$('#date_cd_utc_lax').text(" UTC - " +moment(dt).utc().format('HH:mm:ss')+"Z");
		}, 1000);

		var chartUpdateDiv = $("#chart5");
		chartUpdateDiv.html("");
		var canvasElement = $('<canvas id = "canvas5"/>')		
